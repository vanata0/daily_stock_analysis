...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.TUSHARE_API_URL': {
    ...
    valueNotes: [
      '必须以 http:// 或 https:// 开头，否则该数据源会初始化失败并自动降级为不可用（不影响其它数据源）。',
      '代理节点需兼容 Tushare Pro 的 HTTP POST JSON 协议。',
      '修改后需重启服务生效。',
    ],
  ...
  'settings.data_source.KPL_ENABLED': {
    title: '启用 KPL 数据源',
    summary: '启用开盘啦（KPL）数据源，覆盖行情、板块概念、涨停情绪、公告资讯等 A 股数据。',
    usage: '需先在本机运行 kpl-unified-client HTTP 服务。默认关闭，关闭时完全不实例化该数据源。',
    valueNotes: [
      'KPL 凭证来自抓包会过期，过期时上游不报错而是返回空数据；系统内置探针会在交易日检测到涨跌家数为空时自动停用该源并回落到其它数据源。',
      'GitHub Actions 环境无法连接本机服务，云端定时任务应保持关闭。',
    ],
    impact: ['开启后 KPL 参与实时行情、日线、板块概念与资讯链路。'],
    notes: ['关闭即可完全回到原有数据源行为，不影响其它配置。'],
  },
  'settings.data_source.KPL_API_BASE': {
    title: 'KPL 服务地址',
    summary: 'kpl-unified-client HTTP 服务的访问地址。',
    usage: '默认 http://127.0.0.1:8010。必须以 http:// 或 https:// 开头。',
    valueNotes: [
      'Docker 部署时容器内的 127.0.0.1 指向容器自身，需改为 http://host.docker.internal:8010。',
      '地址不可达时该数据源自动降级，不会中断分析流程。',
    ],
    impact: ['影响所有 KPL 接口请求的目标地址。'],
    notes: ['修改后需重启服务生效。'],
  },
  'settings.data_source.KPL_PRIORITY': {
    title: 'KPL 数据源优先级',
    summary: 'KPL 在数据源链中的尝试顺序，数字越小越早尝试。',
    usage: '默认 -1，表示启用后排在所有数据源最前面。',
    valueNotes: ['调大该值可让 KPL 退居备用位置，由原有数据源优先。'],
    impact: ['影响日线等按优先级降级的数据链路。'],
    notes: ['该配置仅在 KPL 已启用时生效。'],
  },
  'settings.data_source.KPL_TIMEOUT': {
    title: 'KPL 请求超时（秒）',
    summary: 'KPL 单次 HTTP 请求的超时秒数。',
    usage: '填写正整数，默认 10。',
    valueNotes: ['服务在本机运行时通常毫秒级返回，超时主要用于防止服务异常时阻塞分析流程。'],
    impact: ['影响 KPL 请求失败前的等待时长。'],
    notes: ['超时后自动回落到其它数据源。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.TUSHARE_API_URL': {
    ...
    valueNotes: [
      'Must start with http:// or https://, otherwise this data source fails to initialize and is automatically disabled (other data sources are unaffected).',
      'The proxy must be compatible with Tushare Pro HTTP POST JSON protocol.',
  ...
  'settings.data_source.KPL_ENABLED': {
    title: 'KPL Data Source',
    summary: 'Enables the KPL (Kaipanla) data source covering A-share quotes, sectors/concepts, limit-up sentiment, filings, and news.',
    usage: 'Requires a local kpl-unified-client HTTP service. Disabled by default; the source is not instantiated at all when off.',
    valueNotes: [
      'KPL credentials come from packet capture and expire. On expiry the upstream returns empty data instead of an error; a built-in probe detects empty market breadth on a trading day, disables this source, and falls back to the existing providers.',
      'GitHub Actions runners cannot reach a local service, so keep this off for cloud scheduled runs.',
    ],
    impact: ['When enabled, KPL participates in realtime quotes, daily K-lines, sector/concept data, and news paths.'],
    notes: ['Turning it off fully restores the previous data source behavior.'],
  },
  'settings.data_source.KPL_API_BASE': {
    title: 'KPL API Base URL',
    summary: 'Base URL of the kpl-unified-client HTTP service.',
    usage: 'Defaults to http://127.0.0.1:8010. Must start with http:// or https://.',
    valueNotes: [
      'Inside Docker, 127.0.0.1 points at the container itself — use http://host.docker.internal:8010 instead.',
      'An unreachable address degrades this source gracefully without interrupting analysis.',
    ],
    impact: ['Applies to all KPL API requests.'],
    notes: ['Restart the service after changing this value.'],
  },
  'settings.data_source.KPL_PRIORITY': {
    title: 'KPL Priority',
    summary: 'Order in which KPL is tried within the data source chain; lower values are tried earlier.',
    usage: 'Defaults to -1, placing KPL ahead of all other sources when enabled.',
    valueNotes: ['Raise this value to keep KPL as a backup behind the existing providers.'],
    impact: ['Affects priority-based fallback chains such as daily K-lines.'],
    notes: ['Only effective when KPL is enabled.'],
  },
  'settings.data_source.KPL_TIMEOUT': {
    title: 'KPL Request Timeout',
    summary: 'Per-request timeout in seconds for KPL HTTP calls.',
    usage: 'Use a positive integer. The default is 10.',
    valueNotes: ['A local service usually responds in milliseconds; this timeout mainly prevents a stalled service from blocking analysis.'],
    impact: ['Affects how long a KPL request waits before failing.'],
    notes: ['On timeout the chain falls back to other data sources.'],
  },
  'settings.data_source.stock_index_remote': {
