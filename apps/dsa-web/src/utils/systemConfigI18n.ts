...
const fieldTitleMap: Record<string, string> = {
  ...
  KPL_ENABLED: '启用 KPL 数据源',
  KPL_API_BASE: 'KPL 服务地址',
  KPL_PRIORITY: 'KPL 数据源优先级',
  KPL_TIMEOUT: 'KPL 请求超时（秒）',
...
const fieldDescriptionMap: Record<string, string> = {
  ...
  MAIRUI_API_KEY: '迈瑞数据个股资金流向接口密钥，提供特大/大/中/小单四档明细；配置后作为资金流主数据源，未配置时自动降级到东财。',
  ...
  KPL_ENABLED: '启用开盘啦（KPL）数据源，需先在本机运行 kpl-unified-client HTTP 服务；默认关闭，关闭时不影响现有数据源。',
  KPL_API_BASE: 'kpl-unified-client HTTP 服务地址，必须以 http:// 或 https:// 开头；Docker 部署时需改用 host.docker.internal。',
  KPL_PRIORITY: 'KPL 在数据源链中的优先级，数字越小越早尝试；默认 -1 表示启用后排在最前。',
  KPL_TIMEOUT: 'KPL 单次 HTTP 请求的超时秒数。',
