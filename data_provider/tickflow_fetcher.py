...
TickFlow is optional and fail-open in this project. Supports three usage modes:

1. **Per-stock daily K-line** (participates in the standard DataFetcherManager
   pipeline).  Uses ``client.klines.get(symbol, period="1d", adjust=...)``.
   Active when TICKFLOW_API_KEY is set; priority controlled by
   TICKFLOW_PRIORITY (default 2).

2. **Realtime quotes**, keeping the existing market review capability and
   exposing advanced TickFlow-only helpers for future consumers.

3. **A-share market review** (called explicitly by DataFetcherManager).
...
class TickFlowFetcher(BaseFetcher):
    """TickFlow-backed optional A-share fetcher — daily K-line + realtime quote + market review."""
