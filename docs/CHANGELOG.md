...
- [修复] Agent 模式下报告误报 `news_context_missing`：AnalysisContextPack 在 Agent 启动前构建，只能看到预取的公告/东财新闻/本地情报池；Agent 运行中自己调用 `search_stock_news` / `search_comprehensive_intel` 取回的新闻晚于该时点，因此三路预取全空时（如 Tushare token 过期 + 东财新闻被汇总噪音过滤 + 本地情报池无记录），即使 LLM 实际读到了新闻，报告仍显示「新闻缺失」。现在 Agent 结束后按工具调用轨迹回填 news 块并重建运行诊断概览。
...
- [修复] KPL 凭证探针改打 `GET /health/credential`（kpl-unified-client 专为下游探测开的健康检查端点，不经过任何业务 Response Model），不再打业务端点 `/market-stats/mood-num-count`。2026-08-11 17:52~08-12 01:15 该业务端点因响应模型必填了一个客户端已改名的字段而稳定 HTTP 500，`is_credential_valid()` 把非 2xx 一律判定不可用，导致 KPL 全部接口（日线/实时行情/板块/涨停池/资金流/研报/基本面）一起短路回落到 Tushare 等其它数据源。新探针按状态码判定（200 可用 / 401 凭证问题 / 503 上游问题 / 429 限流不算失效），字段语义订正不会再影响探针本身；交易日 09:30 前与非交易日两条误报豁免逻辑保持不变。
...
- [改进] 按报告期取的全市场表加进程内 TTL 缓存（300 秒，跨 adapter 实例共享，上限 32 项）：单期业绩报表近 6000 行、秒级返回，批量分析时每只股票重拉一遍会吃满 fundamental 抓取预算。
- [新功能] 筹码分布接入 KPL 作为首选源，Tushare 与 AkShare 完整保留兜底：上游为本地计算（开盘啦对成本分布无 HTTP/Socket 接口，App 自身也是拿日 K 本地算的，该端点用三角形分布+换手衰减的同族算法基于 1000 根日线复现），因此不依赖将失效的 Tushare 凭证。三方在 2026-07-27 收盘同一时点对拍，均通过自洽性检验（70% 区间 ⊆ 90% 区间、均成本落在 90% 区间内、集中度等于 (高-低)/(高+低)、获利比与收盘价在区间中的位置方向一致），KPL 与 AkShare 系统性接近而 Tushare 离群；三者都是算法输出无绝对真值，故不主张谁更准，选它排最前的理由是耗时 0.1 秒（AkShare 6~11 秒、Tushare 17~28 秒）、结果稳定可复现、且与既定兜底源 AkShare 趋同使切换时数值跳变更小。上游 `decay` 参数有意不透传——0.8~1.2 会让获利比在 0.197~0.284 间摆动。
- [修复] KPL 凭证探针在交易日开盘前误判失效：上游会在当天早盘前把上一交易日的涨跌家数清零，此时 rise/fall 全 0 是正常状态，而探针只排除了非交易日，导致每个交易日 09:30 之前 KPL 全线不可用并每天误发一次凭证失效告警（实测 2026-07-28 09:01 该端点返回全 0 且 errcode='0'，凭证完全有效）。现在交易日 09:30 之前同样不做失效判定。
- [修复] 个股所属板块不再由 KPL 承担，回落东财行业口径：KPL 的 `/plate-list/stock-sector-v2` 按**板块当日涨跌幅降序**返回，让「今天哪个板块涨得多」决定「这只股票属于什么」，且涨跌幅盘中变化会让顺序跟着变，而下游只取前 5 条，导致同一标的不同时刻的分析结果不可复现；返回的 44 条几乎全是概念标签，缺申万行业层级。实测 002364（电源设备公司）KPL 前 5 为「拼多多概念/杭州/电源/光储充一体化/通信」，东财为「电力设备/其他电源设备Ⅲ/其他电源设备Ⅱ/浙江板块/换电概念」。KPL 的板块涨跌排行、实时行情、日线等能力不受影响。
- [修复] Pytdx 选服只看 TCP 连接是否成功，会锁死在「僵尸节点」上：通达信公开服务器中存在 TCP 与协议握手均正常、但任何 K 线查询都返回空列表的节点，旧实现 `api.connect()` 返回 True 即 break，于是永远停在服务器列表里第一个僵尸节点，真正可用的节点排在后面也轮不到，对外表现为「Pytdx 未查询到 <code> 的数据」，看起来像标的问题实为选错服务器；连接全部失败时还会付出 5 秒超时 × N 台的代价（实测 25 秒）。现在连接成功后补一次最小取数探针，未返回数据则断开并继续试下一台。
- [修复] Tushare 的筹码分布数据被标成 AkShare 来源：`ChipDistribution.source` 的 dataclass 默认值是 `akshare`，`TushareFetcher` 构造时未显式赋值，使运行流与报告里的来源追溯失真。
- [修复] 实时行情成交量单位错误导致 volume 比实际小 100 倍：`UnifiedRealtimeQuote.volume` 契约为「股」，但 Tushare 旧版实时接口把上游本就是「股」的数值 `//100` 转成了「手」，Efinance 与 Tushare Pro 接口则直接透传上游按「手」计的原值。实测中恒电气（002364）同一时刻 `amount` 与其它源一致（996,306,389）而 volume 仅 260,124，`amount/volume` 反推均价 3830 是现价 38.28 的 100 倍；修复后三源反推均价一致落在 38.30。港股与美股本就按股报量，不做换算。该缺陷会静默污染量比、换手率及一切基于成交量的判断。
...
- [新功能] KPL 数据源补齐板块与涨停维度：个股所属板块、股票名称、板块涨跌排行（取自全市场 `pc-plate-ranking` 并按涨跌幅重排，而非只含 8~10 条的「精选强势板块」）、涨停池与连板梯队（按连板数分档聚合，单档失败不影响其它档位）。概念排行有意不实现——上游只提供题材热度分而非涨跌幅，填入 `change_pct` 会被下游误读为涨跌幅，保持降级到既有数据源。
