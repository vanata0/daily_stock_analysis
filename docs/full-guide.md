...
| `TUSHARE_API_URL` | Tushare Pro 自定义接入地址（Web 设置页可编辑）；留空（或未设置/空白）时使用官方端点 `http://api.tushare.pro`，仅在需通过公司内网代理、跨境网络或自建镜像时填写 `http://` 或 `https://` 开头的完整地址，非法前缀会导致该数据源初始化失败并自动降级 | 可选 |
...
| `TUSHARE_API_URL` | Tushare Pro 自定义接入地址（Web 设置页可编辑）；留空时使用官方端点 `http://api.tushare.pro`，仅在需通过公司内网代理、跨境网络或自建镜像时填 `http://` 或 `https://` 开头的完整地址 | `http://api.tushare.pro` | 可选 |
