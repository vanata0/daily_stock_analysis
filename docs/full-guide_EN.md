...
| `TUSHARE_API_URL` | Tushare Pro custom endpoint (editable in the Web settings page); when unset/empty defaults to the official `http://api.tushare.pro`. Set to a `http://` or `https://` URL only when routing through a corporate proxy, cross-border network, or a self-hosted mirror — an invalid prefix causes this data source to fail initialization and degrade gracefully | Optional |
...
| `TUSHARE_API_URL` | Tushare Pro custom endpoint (editable in the Web settings page); defaults to `http://api.tushare.pro` when unset/empty. Set only when routing through a corporate proxy, cross-border network, or a self-hosted mirror (must start with `http://` or `https://`). | `http://api.tushare.pro` | Optional |
...
| `KPL_PRIORITY` | KPL data source priority; lower values are tried earlier; defaults ahead of Tushare. | `-2` | Optional |
