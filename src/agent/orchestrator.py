    ...
    def _execute_pipeline(
        ...
        while index < len(agents):
            ...
            if (
                ...
                _tech_timeout_exhausted = (
                    timeout_s
                    and _remaining_after_tech is not None
                    and _remaining_after_tech <= 0
                )
                _tech_budget_guard_triggered = (
                    timeout_s
                    and _remaining_after_tech is not None
                    and _remaining_after_tech < stage_min_budget_s
                    and not _tech_timeout_exhausted
                )
                if _tech_timeout_exhausted:
                    logger.error(
                        "[Orchestrator] pipeline timed out before stage 'intel'",
                    )
                    self._record_degraded_event(
                        ctx,
                ...
                if _tech_budget_guard_triggered:
                    logger.warning(
