...
class PortfolioService:
    ...
    def _fetch_realtime_position_price(symbol: str) -> Tuple[Optional[float], Optional[str]]:
        try:
            ...
            # A fresh manager per call (see #1803): sharing one instance across the
            # parallel executor's workers would re-serialize requests through its
            # internal per-fetcher locks, defeating the parallel fetch. price_only
            # skips the slow per-stock Tencent supplement of fields (turnover/PE/
            # 量比…) the snapshot discards — that's what actually made this slow.
            fetcher_manager = DataFetcherManager()
            quote = fetcher_manager.get_realtime_quote(symbol, log_final_failure=False, price_only=True)
        except Exception as exc:
