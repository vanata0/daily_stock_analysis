...
# 内建选股默认 max_tokens=2048；但 DSA 选股常用的思考型模型（如 deepseek-v4-flash）会把
# reasoning_content 计入同一 completion token 预算。候选池接近 DSA_SCREENING_LLM_MAX_CANDIDATES
# 且排序 schema 字段较多时，思考阶段可能耗尽全部预算，导致正文 content 为空、JSON 解析失败
# 并整体降级为 screen_score 排序。复测同规模候选池实际需要约 3400 tokens 才能思考完并输出完整
# JSON，这里预留安全余量。
DSA_SCREENING_LLM_MAX_TOKENS = 8192
...
def _build_screening_runtime_env(config: Config, *, max_results: Optional[int] = None) -> Dict[str, str]:
    ...
    put_default("LLM_MAX_TOKENS", str(DSA_SCREENING_LLM_MAX_TOKENS))
...
class DsaEastMoneyHotspotProvider:
    ...
    def _fetch_hotspot_spot_quote_map(self, codes: List[str], *, limit: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug("Screening hotspot quote map skipped: %s", exc)
            return {}
        if tushare is None:
            return {}
        quote_by_code: Dict[str, Any] = {}
